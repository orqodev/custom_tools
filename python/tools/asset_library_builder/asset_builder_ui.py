"""
Asset Library Builder UI - PySide6 dialog for building USD asset assemblies

Creates proper USD asset structure (payload → geo + mtl) with texture variants and instancing.
...
class AssetLibraryBuilderUI(QtWidgets.QDialog):
    """Simple UI for Asset Library Builder."""

    def __init__(self, parent=None):
        super().__init__(parent)

        self.setWindowTitle("Asset Library Builder")
    ...
    def _setup_ui(self):
        ...
        header = QtWidgets.QLabel(
            "<h2>Asset Library Builder</h2>"
            "<p>Creates production-ready USD asset libraries with proper structure</p>"
        )
        ...
        self.source_parts_edit.setPlaceholderText("Path to parts folder (e.g., /parts/MyAsset/)...")
        ...
        self.asset_name_edit.setPlaceholderText("MyAsset")
        form_layout.addRow("Asset Name:", self.asset_name_edit)

        # Add auto-detect button
        auto_detect_layout = QtWidgets.QHBoxLayout()
        auto_detect_btn = QtWidgets.QPushButton("Auto-detect from folder name")
        ...
        # Status label
        self.status_label = QtWidgets.QLabel("Ready to build")
    ...
    def _build_assembly(self):
        ...
        try:
            ...
            payload_path = build_from_parts(
                ...
                progress_callback=self._update_progress  # Connect progress callback
            )

            # Success
            self.progress_bar.setVisible(True)  # Keep visible to show 100%
            ...
            msg.setText("<b>Asset Assembly Created Successfully!</b>")
...
def show_ui():
    """Show the Asset Library Builder UI."""
    # Get parent widget (Houdini main window)
    parent = hou.qt.mainWindow()

    # Close existing dialog if open
    for widget in QtWidgets.QApplication.topLevelWidgets():
        if isinstance(widget, AssetLibraryBuilderUI):
            widget.close()

    # Create and show dialog
    dialog = AssetLibraryBuilderUI(parent)
    dialog.show()
