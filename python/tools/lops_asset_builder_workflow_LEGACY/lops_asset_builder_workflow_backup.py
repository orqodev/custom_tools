"""
LOPS Asset Builder Workflow - Backup File

This is a backup of the original LOPS Asset Builder Workflow file.
"""
...
def toggle_lops_asset_builder_workflow():
    ...
    print("LOPS Asset Builder Workflow - Backup file loaded")
    print("This is a backup file. Please use the main workflow instead.")
    return False
...
class _AssetGroupsDialog(QtW.QDialog):
    ...
    def load_template(self):
        ...
        try:
            ...
            # Clear ALL existing groups completely
            while len(self.group_widgets) > 0:
                self.remove_group_widget(self.group_widgets[-1])

            # Load asset scope
            if 'asset_scope' in template_data:
                self.asset_scope_edit.setText(str(template_data['asset_scope']))

            # Create ALL groups uniformly in one go
            if template_data['groups']:
                for group_index, group_data in enumerate(template_data['groups']):
                    ...
                    if 'group_name' in group_data:
                        new_group.name_edit.setText(str(group_data['group_name']))

                    # Set texture folder
                    if 'texture_folder' in group_data:
                        new_group.materials_folder_edit.setText(str(group_data['texture_folder']))

                    # Set asset paths with improved clearing logic
                    if 'asset_paths' in group_data:
                        ...
                        # More robust path clearing - create a copy of the list to avoid modification during iteration
                        path_rows_to_remove = list(new_group.path_rows)
                        for path_row in path_rows_to_remove:
                            new_group.remove_path_row(path_row)

                        # Verify all paths are cleared
                        if len(new_group.path_rows) > 0:
                            # Force clear any remaining paths
                            new_group.path_rows.clear()

                        # Add all paths (create path rows as needed)
                        for i, path in enumerate(paths):
                            if not isinstance(path, str):
                                raise ValueError(f"Invalid template format: group {group_index + 1} path at index {i} must be a string")
                            new_group.add_path_row()
                            # Verify the path row was added successfully
                            if len(new_group.path_rows) > 0:
                                new_group.path_rows[-1].set_path(str(path))
